...
# Unload here to reset working dir
if(any(grepl("package:here", search()))) {
  message("Detaching package 'here'")
  detach("package:here", unload=TRUE)
} else {
  message("here not loaded")
}
...
trialExamplePlotExp1 <- trialExample %>%
  ggplot(data=., aes(x=time/1000, y=YNorm, color=trialNumYAxis)) +
    ...
    ylim(-700, 700) +
    xlab("Time (s)") +
    ylab("Normalized Y position (pixels)") +
...
examplePlotExp1 <- example %>%
  ...
  facet_grid(~phase)
examplePlotExp1
ggsave('figures/examplePlotExp1.pdf', examplePlotExp1, width=10, height=5)
...
trialExampleExp2 <- trialExample %>%
  ggplot(data=., aes(x=time/1000, y=YNorm, color=trialNumYAxis)) +
    ...
    facet_grid(phase~.)
trialExampleExp2
ggsave('figures/trialExampleExp2.pdf', trialExampleExp2.pdf, width=7, height=4)
...
examplePlotExp2 <- example %>%
  ggplot(data=., aes(x=X, y=YNorm, color=time/1000, group=time)) +
    ...
    theme()
examplePlotExp2
ggsave('figures/examplePlotExp2.pdf', examplePlotExp2, width=10, height=5)
...
# Attention: if this is not working detach the package plyr...
# Without roving
trainingEffectsExp1 <- data.mountain.yAxis %>%
  ...
  mutate(metric=c(rep("Score", 10), rep("Trial duration (s)", 10), rep("Error (%)", 10)))
...
trainingEffectsExp2 <- data.mountain.yAxis %>%
  ...
  filter((first=="meanFirstScore" & last=="meanLastScore") |
          (first=="meanFirstDur" & last=="meanLastDur") |
           (first=="meanFirstAcc" & last=="meanLastAcc")) %>%
  mutate(metric=c(rep("Score", 14), rep("Trial duration (s)", 14), rep("Error (%)", 14)))
...
mean(effectIdDet$diff[effectIdDet$roving==FALSE])
mean(effectIdDet$diff[effectIdDet$roving==TRUE])

wilcox.test(effectIdDet$diff[effectIdDet$roving==FALSE],
            effectIdDet$diff[effectIdDet$roving==TRUE],
            paired=FALSE)
...
plotMeanThresholdsRov <- thresholdsAllLongRov %>%
  ...
  summarise(mean=mean(threshold),
            N=length(threshold),
            se=sd(threshold)/sqrt(N)) %>%
  ggplot(aes(x=session, y=mean, color=condition, group=condition), alpha = 0.3) +
    # geom_line(size=1.3) +
    geom_point(size=5) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.1, size=1.5) +
    ...
    scale_color_manual(name="", breaks=c('Identification', 'Detection'),
                          values=c('black', 'gray70')) +
    simpleTheme
plotMeanThresholdsRov
ggsave('figures/plotMeanThresholdsRov.pdf', plotMeanThresholdsRov, width=8, height=4)
...
plotMeanThresholdsNoRov <- thresholdsAllLongNoRov %>%
  ...
  summarise(mean=mean(threshold),
            ...
            sd=list(threshold),
            se=sd(threshold)/sqrt(N)) %>%
  ggplot(aes(x=session, y=mean, color=condition, group=condition)) +
    # geom_line(size=1.3) +
    geom_point(size=5) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.1, size=1.5) +
    scale_y_continuous(trans='log2', breaks=c(1, 2, 4, 8, 16), limits=c(0.5, 18)) +
...
ggsave('figures/plotMeanThresholdsNoRov.pdf', plotMeanThresholdsNoRov, width=8, height=4)
...
plotMeanThresholdsRovLongi <- thresholdsLongiAllLong %>%
  ...
  ggplot(aes(x=session, y=mean, color=condition, group=condition), alpha = 0.3) +
    # geom_line(size=1.3) +
    geom_point(size=5) +
    geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.5, size=1.5) +
...
plotMeanThresholdsRovLongiInd <- thresholdsLongiAllLong %>%
  ...
  mutate(session=factor(session, label=c('PRE1', 'PRE2', 'POST1', 'POST2',
                                         ...
                                         'PRE7', 'PRE8', 'POST7', 'POST8',
                                         'PRE9', 'PRE10', 'POST9', 'POST10'))) %>%
  group_by(name, session, condition) %>%
  summarise(mean=mean(threshold),
            N=length(threshold),
            se=sd(threshold)/sqrt(N)) %>%
  ggplot(aes(x=session, y=mean, color=condition, group=condition), alpha = 0.3) +
    ...
    geom_point(size=3) +
    # geom_errorbar(aes(ymin=mean-se, ymax=mean+se), width=0.1, size=1.5) +
    scale_y_continuous(trans='log2', breaks=c(1, 2, 4, 8, 16, 50)) +
...
plotMeanThresholdsRovLongiInd
ggsave('figures/plotMeanThresholdsRovLongiInd.pdf', plotMeanThresholdsRovLongiInd, width=12, height=6)
```
...
yAxisAccRovCompleted <- yAxisAccRov %>%
  ...
  ggplot(aes(x=percentCompletedSmooth, y=resultMean)) +
    geom_point(size=5) +
    geom_errorbar(aes(ymin=resultMean-se, ymax=resultMean+se), width=0.04, size=1.5) +
    stat_smooth(method = "lm", formula = y ~ x, size = 1.6, se=FALSE, color="black") +
    geom_hline(aes(yintercept=(2^(mean/1200)-1)*100, linetype="Detection"),
...
yAxisDurRovCompleted <- yAxisDurRov %>%
  ...
  ggplot(aes(x=percentCompletedSmooth, y=mean)) +
    ...
    stat_smooth(method = "lm", formula = y ~ x, size = 1.6, se=FALSE, color="gray70") +
    xlab("Proportion of training completed") +
    ylab("Duration (s)") +
    simpleTheme +
    scale_x_continuous(breaks=seq(0, 1, 0.1)) +
...
```{r, echo=FALSE, message=FALSE, warning=FALSE, results='asis', width=3, height=4}
# Training accuracy in percent of total
yAxisAccRovCompletedLongi <- yAxisAccLongi %>%
  ...
  ggplot(aes(x=percentCompletedSmooth, y=mean)) +
    ...
    geom_hline(aes(yintercept=(2^(mean/1200)-1)*100, linetype="Identification"),
                thresholdsLongiAllLong %>%
                  filter(condition=='identification') %>%
                  summarise(mean=mean(threshold)),
               size=1.5,
               color='gray70') +
    xlab("Proportion of training completed") +
...
trainingEffectsLongi <- data.mountain.yAxis.longi %>%
  ...
  filter((first=="meanFirstScore" & last=="meanLastScore") |
          (first=="meanFirstDur" & last=="meanLastDur") |
           (first=="meanFirstAcc" & last=="meanLastAcc")) %>%
  mutate(metric=c(rep("Score", 4), rep("Trial duration (s)", 4), rep("Error (%)", 4))) %>%
  mutate(name=factor(name, labels=c('p1', 'p2', 'p3', 'p4')))
...
wilcox.test(trainingEffectsLongi$firstVal[trainingEffectsLongi$metric=="Score"],
trainingEffectsLongi$lastVal[trainingEffectsLongi$metric=="Score"], paired=TRUE)
```
...
detIdPretestRovLongi <- thresholdsLongiAllLong %>%
  ...
  ggplot(aes(detection, identification, color=name)) +
    ...
    ylab("Identification (%)") +
    scale_color_discrete(name="Participants") +
    scale_x_continuous(trans='log', breaks=c(0.1, 0.2, 0.5, 1, 2, 5, 10), limits=c(0.1, 50)) +
    scale_y_continuous(trans='log', breaks=c(0.1, 0.2, 0.5, 1, 2, 5, 10), limits=c(0.1, 50)) 
